...
static void tty_notify(const char *tty,  const int state) {
	JNIEnv *env;
    BOOL attached =  jni_attach_thread(&env);
    jstring jTTY = (*env)->NewStringUTF(env,tty);
	(*env)->CallStaticVoidMethod(env,JNI_SERIAL_HELPER.clazz,JNI_SERIAL_HELPER.jNotify,jTTY,(jint)state);
    (*env)->DeleteLocalRef(env,jTTY);
    if(attached)
        jni_detach_thread();

}

static int tty_available(const char *tty) {
    JNIEnv *env;
    BOOL attached =  jni_attach_thread(&env);
    jstring jTTY = (*env)->NewStringUTF(env,tty);
    int r = (*env)->CallStaticIntMethod(env,JNI_SERIAL_HELPER.clazz,JNI_SERIAL_HELPER.jAvailable,jTTY);
    (*env)->DeleteLocalRef(env,jTTY);
    if(attached)
        jni_detach_thread();
    return r;
...
static int tty_read(const char *tty, void *buf, const int offset, const int size) {
    JNIEnv *env;
    BOOL attached =  jni_attach_thread(&env);
    jstring jTTY = (*env)->NewStringUTF(env,tty);
    jbyteArray a = (*env)->NewByteArray(env,size);
    int r = (*env)->CallStaticIntMethod(env,JNI_SERIAL_HELPER.clazz,JNI_SERIAL_HELPER.jRead,jTTY,a,0,(jint)size);
    (*env)->DeleteLocalRef(env,jTTY);
    if(r > 0) {
        jbyte *res = (*env)->GetByteArrayElements(env,a,NULL);
        uint8_t *p = (uint8_t *)buf;
        memcpy(&p[offset],res,r);
        (*env)->ReleaseByteArrayElements(env,a,res,JNI_ABORT);
    }
...
static void tty_write(const char *tty, void *buf, const int offset, const int size) {
    JNIEnv *env;
    BOOL attached =  jni_attach_thread(&env);
    jstring jTTY = (*env)->NewStringUTF(env,tty);
    jbyteArray a = (*env)->NewByteArray(env,size);
    (*env)->SetByteArrayRegion(env,a,0,(jint)size,(jbyte *)buf);
    (*env)->CallStaticVoidMethod(env,JNI_SERIAL_HELPER.clazz,JNI_SERIAL_HELPER.jWrite,jTTY,a,0,(jint)size);
    (*env)->DeleteLocalRef(env,jTTY);
    (*env)->DeleteLocalRef(env,a);
    if(attached)
        jni_detach_thread();
...
static void JNICALL jni_freerdp_set_serial_class(JNIEnv *env, jclass self, jclass helper) {
    JNI_SERIAL_HELPER.clazz = (jclass)(*env)->NewGlobalRef(env, helper);
    JNI_SERIAL_HELPER.jAvailable = (*env)->GetStaticMethodID(env,helper,"available","(Ljava/lang/String;)I");
    JNI_SERIAL_HELPER.jRead = (*env)->GetStaticMethodID(env,helper,"read","(Ljava/lang/String;[BII)I");
    JNI_SERIAL_HELPER.jWrite = (*env)->GetStaticMethodID(env,helper,"write","(Ljava/lang/String;[BII)V");
	JNI_SERIAL_HELPER.jNotify = (*env)->GetStaticMethodID(env,helper,"notifyTTY","(Ljava/lang/String;I)V");
    JNI_SERIAL_HELPER.helper.available = &tty_available;
    JNI_SERIAL_HELPER.helper.read = &tty_read;
    JNI_SERIAL_HELPER.helper.write = &tty_write;
	JNI_SERIAL_HELPER.helper.notify = &tty_notify;
    setSerialHelper(&JNI_SERIAL_HELPER.helper);
}


static jstring JNICALL jni_freerdp_get_error(JNIEnv* env, jclass cls, jlong handle) {
   UINT32 err = freerdp_get_last_error(((freerdp *)handle)->context);
   return (*env)->NewStringUTF(env, freerdp_get_last_error_string(err));
}

static jstring JNICALL jni_freerdp_get_jni_version(JNIEnv* env, jclass cls)
{
	if(!g_JavaVm) {
	  WLog_DBG(TAG, "Setting up VM");
...
static JNINativeMethod methods[] = {
	...
	{ "freerdp_set_serial_class", "(Ljava/lang/Class;)V", &jni_freerdp_set_serial_class },
	{ "freerdp_get_error","(J)Ljava/lang/String;",&jni_freerdp_get_error}
};
